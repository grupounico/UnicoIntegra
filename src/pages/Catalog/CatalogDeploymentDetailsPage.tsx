...
import { cancelDeployment, getDeployment, retryDeployment, retryUnit, runUnit, subscribeToDeployment, type Deployment, type DeploymentEvent, type DeploymentStatus, type DeploymentUnit } from '../../services/catalogDeployment.service';
...
const eventLabels: Record<string, string> = {
  ...
  unit_configuration_updated: 'Configuração da unidade corrigida',
  hub_run_status_changed: 'Status da carga atualizado', hub_run_delayed: 'Carga demorando mais que o esperado',
  hub_run_poll_retry: 'Nova consulta ao Hub agendada', hub_run_reconciled: 'Carga anterior reconciliada',
  hub_run_not_found: 'Carga ainda não localizada no Hub',
};
...
function runStatusLabel(status: string | null | undefined) {
  const labels: Record<string, string> = { queued: 'Na fila', running: 'Em processamento', published: 'Publicado', delayed: 'Processamento demorado', failed: 'Falhou', cancelled: 'Cancelado' };
  return status ? labels[status] || status : 'Aguardando';
}

function UnitStatusCard({ deployment, unit, busyAction, onAction }: { deployment: Deployment; unit: DeploymentUnit; busyAction: string; onAction: (action: 'retry' | 'run', unit: DeploymentUnit) => void }) {
  const busy = busyAction.endsWith(unit.id);
  const run = unit.hubRun;
  return <article className="rounded-xl border border-[#dbe3ef] bg-white p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div className="flex min-w-0 items-start gap-3"><span className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-slate-100 text-slate-600"><Store className="size-5" /></span><div className="min-w-0"><h3 className="truncate text-sm font-semibold text-slate-950">{unit.name}</h3><p className="mt-1 text-xs text-slate-500">{unit.code} · {formatCnpj(unit.cnpj)}{unit.isInitial ? ' · Inicial' : ''}</p></div></div><StatusBadge status={unit.status} /></div><dl className="mt-5 grid grid-cols-2 gap-x-5 gap-y-4 border-t border-slate-200 pt-4 text-xs"><div><dt className="text-slate-500">ID Alpha7</dt><dd className="mt-1 font-semibold text-slate-800">{unit.sourceUnitId}</dd></div><div><dt className="text-slate-500">Itens válidos</dt><dd className="mt-1 font-semibold tabular-nums text-slate-800">{unit.latestValidRows?.toLocaleString('pt-BR') ?? '—'}</dd></div><div><dt className="text-slate-500">ID no Hub</dt><dd className="mt-1 truncate font-mono text-[11px] font-semibold text-slate-800">{unit.hubSellerUnitId ?? '—'}</dd></div><div><dt className="text-slate-500">Tenant</dt><dd className="mt-1 truncate font-mono text-[11px] font-semibold text-slate-800">{unit.unicommerceTenantId ?? '—'}</dd></div></dl>{run ? <div className={`mt-4 rounded-lg border px-3 py-3 text-xs ${run.status === 'delayed' ? 'border-amber-200 bg-amber-50 text-amber-900' : run.status === 'failed' ? 'border-rose-200 bg-rose-50 text-rose-800' : 'border-blue-100 bg-blue-50 text-blue-900'}`}><div className="flex items-center justify-between gap-3"><span className="font-semibold">{runStatusLabel(run.status)}</span><code className="text-[11px]">Run {run.runId.slice(0, 8)}</code></div><dl className="mt-3 grid grid-cols-3 gap-3"><div><dt className="opacity-70">Processados</dt><dd className="mt-1 font-semibold tabular-nums">{run.processedRows?.toLocaleString('pt-BR') ?? '—'}</dd></div><div><dt className="opacity-70">Válidos</dt><dd className="mt-1 font-semibold tabular-nums">{run.validRows?.toLocaleString('pt-BR') ?? '—'}</dd></div><div><dt className="opacity-70">Publicados</dt><dd className="mt-1 font-semibold tabular-nums">{run.publishedRows?.toLocaleString('pt-BR') ?? '—'}</dd></div></dl><p className="mt-3 opacity-70">Última consulta: {formatDate(run.lastPolledAt)}</p></div> : null}{unit.lastErrorMessage ? <div className="mt-4 rounded-lg bg-rose-50 px-3 py-2.5 text-xs leading-5 text-rose-700">{unit.lastErrorMessage}</div> : null}<div className="mt-4 flex flex-wrap gap-2">{['draft', 'failed', 'partially_failed', 'monitoring_timeout', 'reconciliation_required'].includes(deployment.status) ? <Link to={`/main/catalogo/${deployment.id}/corrigir?unit=${unit.id}`} className={secondaryButtonClass}><PencilLine className="size-4" />Corrigir dados</Link> : null}{unit.retryable ? <button type="button" onClick={() => onAction('retry', unit)} disabled={busy} className={secondaryButtonClass}>{busy ? <Loader2 className="size-4 animate-spin" /> : <RotateCcw className="size-4" />}Tentar unidade</button> : null}{unit.hubIntegrationId && ['failed', 'catalog_active'].includes(unit.status) ? <button type="button" onClick={() => onAction('run', unit)} disabled={busy} className={secondaryButtonClass}>{busy ? <Loader2 className="size-4 animate-spin" /> : <Play className="size-4" />}Executar nova carga</button> : null}{!unit.retryable && unit.status === 'failed' && deployment.status === 'failed' ? <span className="self-center text-xs text-slate-500">Corrija a origem antes de tentar novamente.</span> : null}</div></article>;
}
...
export default function CatalogDeploymentDetailsPage() {
  ...
  async function handleUnitAction(action: 'retry' | 'run', unit: DeploymentUnit) {
    if (!deployment) return;
    setBusyAction(`${action}-${unit.id}`);
    setActionMessage('');
    try {
      const updated = action === 'retry' ? await retryUnit(deployment.id, unit.id, requestedBy) : await runUnit(deployment.id, unit.id, requestedBy);
      setDeployment(updated);
